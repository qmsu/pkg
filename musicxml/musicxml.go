...
func Decode(musicxmlFile string) (musicXmlDoc MusicXmlDoc, err error) {
	b, err := os.ReadFile(musicxmlFile)
	if err != nil {
		return musicXmlDoc, err
	}
	err = xml.Unmarshal(b, &musicXmlDoc)
	if err != nil {
		return musicXmlDoc, err
	}
	return musicXmlDoc, nil
}
